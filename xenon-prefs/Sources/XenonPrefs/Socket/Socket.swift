...
public class Socket: SocketReader, SocketWriter {
	...
	public static let SOCKET_MAXIMUM_SSL_READ_BUFFER_SIZE = 8_000_000
	...
	public struct Signature: CustomStringConvertible {
		...
		public var description: String {
			"Signature: family: \(self.protocolFamily), type: \(self.socketType), protocol: \(self.proto), address: \(self.address as Socket.Address?), hostname: \(self.hostname as String?), port: \(self.port), path: \(String(describing: self.path)), bound: \(self.isBound), secure: \(self.isSecure)"
		}
		...
		public init?(protocolFamily: Int32, socketType: Int32, proto: Int32, address: Address?) throws {
			guard let family = ProtocolFamily.getFamily(forValue: protocolFamily),
				...
				throw Error(
					code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS, reason: "Bad family, type or protocol passed."
				)
			}

			// Validate the parameters...
			if type == .stream {
				guard pro == .tcp || pro == .unix
				else {
					throw Error(
						code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS,
						reason: "Stream socket must use either .tcp or .unix for the protocol."
					)
				}
			}
			if type == .datagram {
				guard pro == .udp || pro == .unix
				else {
					throw Error(
						code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS,
						reason: "Datagram socket must use .udp or .unix for the protocol."
					)
				}
			}
		...
		/// - Returns: New Signature instance
		///
		public init?(socketType: SocketType, proto: SocketProtocol, address: Address, hostname: String?, port: Int32?)
			throws
		{
			// Validate the parameters...
			if socketType == .stream {
				guard proto == .tcp || proto == .unix
				else {
					throw Error(
						code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS,
						reason: "Stream socket must use either .tcp or .unix for the protocol."
		...
		///
		public init?(
			protocolFamily: ProtocolFamily, socketType: SocketType, proto: SocketProtocol, hostname: String?,
			port: Int32?
		) throws {
			// Make sure we have what we need...
			guard let _ = hostname,
			      let port = port, protocolFamily == .inet || protocolFamily == .inet6
			else {
				throw Error(
					code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS,
					reason: "Missing hostname, port or both or invalid protocol family."
				)
			}
		...
		public init?(socketType: SocketType, proto: SocketProtocol, path: String?) throws {
			...
			// Validate the length...
			guard lengthOfPath < MemoryLayout.size(ofValue: remoteAddr.sun_path)
			else {
			...
			#if !os(Linux)
				remoteAddr.sun_len = UInt8(
					MemoryLayout<UInt8>.size + MemoryLayout<sa_family_t>.size + path.utf8.count + 1
				)
			#endif
		...
		///
		internal init?(
			protocolFamily: Int32, socketType: Int32, proto: Int32, address: Address?, hostname: String?, port: Int32?
		) throws {
			...
			if type == .datagram {
				guard pro == .udp
					...
					throw Error(
						code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS,
						reason: "Datagram socket must use .udp for the protocol."
					)
				}
			}
		...
		internal func unixAddress() throws -> (UnsafeMutablePointer<UInt8>, Int) {
			...
			if self.path == nil {
				throw Error(
					code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS, reason: "Specified path contains zero (0) bytes."
				)
			}

			let utf8 = self.path!.utf8
	...
	public struct Error: Swift.Error, CustomStringConvertible {
		...
		init(bufferSize: Int) {
			self.init(
				code: Socket.SOCKET_ERR_RECV_BUFFER_TOO_SMALL,
				reason: "Socket has an invalid buffer, the size is too small"
			)
	...
	///
	var readBuffer = UnsafeMutablePointer<CChar>.allocate(
		capacity: Socket.SOCKET_DEFAULT_READ_BUFFER_SIZE
	)

	///
	/// Internal Storage Buffer initially created with `Socket.SOCKET_DEFAULT_READ_BUFFER_SIZE`.
	///
	var readStorage = NSMutableData(capacity: Socket.SOCKET_DEFAULT_READ_BUFFER_SIZE)!
	...
	public var delegate: SSLServiceDelegate? {
		didSet {
			...
			if self.delegate != nil {
				self.readBufferSize = Socket.SOCKET_DEFAULT_SSL_READ_BUFFER_SIZE
			}
		}
	}
	...
	public var readBufferSize: Int = Socket.SOCKET_DEFAULT_READ_BUFFER_SIZE {
		...
		didSet {
			...
			if self.readBufferSize < Socket.SOCKET_MINIMUM_READ_BUFFER_SIZE {
				self.readBufferSize = Socket.SOCKET_MINIMUM_READ_BUFFER_SIZE
			}

			if self.readBufferSize != oldValue {
				#if swift(>=4.1)
					self.readBuffer.deinitialize(count: self.readBufferSize)
					self.readBuffer.deallocate()
					self.readBuffer = UnsafeMutablePointer<CChar>.allocate(capacity: self.readBufferSize)
					self.readBuffer.initialize(to: 0)
				#else
					self.readBuffer.deinitialize()
					self.readBuffer.deallocate(capacity: oldValue)
					self.readBuffer = UnsafeMutablePointer<CChar>.allocate(capacity: self.readBufferSize)
					self.readBuffer.initialize(to: 0, count: self.readBufferSize)
				#endif
			}
		}
	}
	...
	public var isActive: Bool {
		self.isListening || self.isConnected
	}
	...
	public var listeningPort: Int32 {
		guard let sig = signature, isListening
		else {
			return Int32(-1)
		}
		return sig.port
	...
	public var remotePort: Int32 {
		guard let sig = signature, sig.port != Socket.SOCKET_INVALID_PORT
		else {
	...
	///
	public class func create(family: ProtocolFamily = .inet, type: SocketType = .stream, proto: SocketProtocol = .tcp)
		throws -> Socket
	{
		// Validate the parameters...
		if type == .stream {
	...
	public class func create(fromNativeHandle nativeHandle: Int32, address: Address?) throws -> Socket {
		guard let addr = address
			...
			throw Error(
				code: Socket.SOCKET_ERR_MISSING_CONNECTION_DATA, reason: "Unable to access socket connection data."
			)
		}
	...
	public func acceptClientConnection(invokeDelegate: Bool = true) throws -> Socket {
		...
		repeat {
			do {
				guard
					let acceptAddress = try Address(addressProvider: { addressPointer, addressLengthPointer in
						#if os(Linux)
					...
					throw Error(
						code: Socket.SOCKET_ERR_WRONG_PROTOCOL,
						reason: "Unable to determine incoming socket protocol family."
					)
				}
		...
		let newSocket = try Socket(fd: socketfd2, remoteAddress: address!, path: signature?.path)
		...
		if invokeDelegate, self.delegate != nil {
			try self.invokeDelegateOnAccept(for: newSocket)
		}
	...
	public func invokeDelegateOnAccept(for newSocket: Socket) throws {
		...
		if !newSocket.needsAcceptDelegateCall {
			throw Error(
				code: Socket.SOCKET_ERR_INVALID_DELEGATE_CALL, reason: "Socket does not need a delegate accept call"
			)
		}
		...
		do {
			...
			guard let sslError = error as? SSLError
			else {
				throw error
			}
	...
	public func connect(to host: String, port: Int32, timeout: UInt = 0, familyOnly: Bool = false) throws {
		...
		if !self.isBlocking && timeout == 0 {
			throw Error(
				code: Socket.SOCKET_ERR_PARAMETER_ERROR, reason: "No timeout set on call for non-blocking socket."
			)
		}

		if port == 0 || port > 65535 {
			throw Error(
				code: Socket.SOCKET_ERR_INVALID_PORT,
				reason: "The port specified is invalid. Must be in the range of 1-65535."
			)
		}
		...
		let socketType: SocketType = self.signature?.socketType ?? .stream
		#if os(Linux)
			var hints = addrinfo(
				ai_flags: AI_PASSIVE,
				ai_family: familyOnly ? self.signature?.protocolFamily.value ?? AF_UNSPEC : AF_UNSPEC,
		...
		while info != nil {
			#if os(Linux)
				socketDescriptor = Glibc.socket(
					info!.pointee.ai_family, info!.pointee.ai_socktype, info!.pointee.ai_protocol
				)
			#else
		...
		if info!.pointee.ai_family == Int32(AF_INET6) {
			...
			throw Error(
				code: Socket.SOCKET_ERR_WRONG_PROTOCOL, reason: "Unable to determine connected socket protocol family."
			)
		}
		...
		if self.isBlocking, timeout > 0 {
			...
			let result = fcntl(socketfd, F_SETFL, flags & ~O_NONBLOCK)
			if result < 0 {
				throw Error(code: Socket.SOCKET_ERR_SET_FCNTL_FAILED, reason: self.lastError())
			}
		}
	...
	public func connect(to path: String) throws {
		...
		guard let sig = self.signature, sig.protocolFamily == .unix
		else {
			throw Error(
				code: Socket.SOCKET_ERR_WRONG_PROTOCOL, reason: "Socket has the wrong protocol, it must be UNIX socket"
			)
		}
	...
	///
	public func listen(
		on port: Int, maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG, allowPortReuse: Bool = true,
		node: String? = nil
	) throws {
		// Make sure we've got a valid socket...
		if self.socketfd == Socket.SOCKET_INVALID_DESCRIPTOR {
		...
		if port == 0 {
			guard
				let addressFromSockName = try Address(addressProvider: { sockaddr, length in
					if getsockname(self.socketfd, sockaddr, length) != 0 {
						throw Error(
							code: Socket.SOCKET_ERR_BIND_FAILED,
							reason: "Unable to determine listening socket address after bind."
						)
					}
				})
			else {
				throw Error(
					code: Socket.SOCKET_ERR_WRONG_PROTOCOL,
					reason: "Unable to determine listening socket protocol family."
				)
			}
	...
	public func listen(on path: String, maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG) throws {
		...
		// Make sure this is a UNIX socket...
		guard let sockSig = self.signature, sockSig.protocolFamily == .unix
		else {
			throw Error(
				code: Socket.SOCKET_ERR_WRONG_PROTOCOL,
				reason: "Socket has the wrong protocol, it must be a UNIX socket"
			)
		}
		...
		guard let signature = sig
			...
			throw Error(
				code: Socket.SOCKET_ERR_BAD_SIGNATURE_PARAMETERS, reason: "Socket contains invalid signature parameters"
			)
		}
	...
	///
	public func listen(
		forMessage buffer: UnsafeMutablePointer<CChar>, bufSize: Int, on port: Int,
		maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG
	) throws -> (bytesRead: Int, address: Address?) {
		// Make sure the buffer is valid...
		if bufSize == 0 {
			throw Error(
				code: Socket.SOCKET_ERR_INVALID_BUFFER, reason: "Socket has an invalid buffer, the size is zero"
			)
		}
		...
		guard self.signature?.isBound == true
		else {
	...
	///
	public func listen(
		forMessage data: NSMutableData, on port: Int, maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG
	) throws -> (bytesRead: Int, address: Address?) {
		// The socket must've been created...
		if self.socketfd == Socket.SOCKET_INVALID_DESCRIPTOR {
			throw Error(code: Socket.SOCKET_ERR_BAD_DESCRIPTOR, reason: "Socket has an invalid descriptor")
		...
		// The socket must've been created for UDP...
		guard let sig = signature,
		      sig.socketType == .datagram, sig.proto == .udp
		else {
			throw Error(code: Socket.SOCKET_ERR_WRONG_PROTOCOL, reason: "This is not a UDP socket.")
		}
	...
	///
	public func listen(
		forMessage data: inout Data, on port: Int, maxBacklogSize: Int = Socket.SOCKET_DEFAULT_MAX_BACKLOG
	) throws -> (bytesRead: Int, address: Address?) {
		// The socket must've been created...
		if self.socketfd == Socket.SOCKET_INVALID_DESCRIPTOR {
			throw Error(code: Socket.SOCKET_ERR_BAD_DESCRIPTOR, reason: "Socket has an invalid descriptor")
	...
	public func read(into buffer: UnsafeMutablePointer<CChar>, bufSize: Int, truncate: Bool = false) throws -> Int {
			...
			if bufSize < self.readStorage.length {
				if truncate {
					memcpy(buffer, self.readStorage.bytes, bufSize)
						...
						self.readStorage = NSMutableData(
							bytes: self.readStorage.bytes.advanced(by: bufSize),
							length: self.readStorage.length - bufSize
						)
					#else
						self.readStorage.replaceBytes(
							in: NSRange(location: 0, length: bufSize), withBytes: nil, length: 0
						)
					#endif
					return bufSize
				} else {
		...
		let count = try readDataIntoStorage()
	...
	///
	public func readDatagram(into buffer: UnsafeMutablePointer<CChar>, bufSize: Int) throws -> (
		bytesRead: Int, address: Address?
	) {
		// Make sure the buffer is valid...
		if bufSize == 0 {
		...
		// Read all available bytes...
		let (count, address) = try readDatagramIntoStorage()
	...
	@discardableResult public func write(from buffer: UnsafeRawPointer, bufSize: Int) throws -> Int {
		...
		if self.socketfd == Socket.SOCKET_INVALID_DESCRIPTOR {
			throw Error(
				code: Socket.SOCKET_ERR_BAD_DESCRIPTOR,
				reason: "The socket is not valid, it must be created and connected"
			)
		}
		...
		while sent < bufSize {
			...
			if self.delegate != nil {
				repeat {
					do {
						...
						guard let err = error as? SSLError
						else {
							throw error
						}
			...
			if s <= 0 {
				if errno == EAGAIN, !self.isBlocking {
	...
	@discardableResult public func write(from data: NSData) throws -> Int {
		...
		return try self.write(from: data.bytes.assumingMemoryBound(to: UInt8.self), bufSize: data.length)
	}
	...
	@discardableResult public func write(from data: Data) throws -> Int {
		...
		if data.count == 0 {
			...
			return try data.withUnsafeBytes { [unowned self] (buffer: UnsafeRawBufferPointer) throws -> Int in
				try self.write(from: buffer.baseAddress!, bufSize: data.count)
			}
		#else
	...
	@discardableResult public func write(from buffer: UnsafeRawPointer, bufSize: Int, to address: Address) throws -> Int {
		...
		return try address.withSockAddrPointer { addressPointer, addressLength -> Int in
			...
			while sent < bufSize {
				...
				#if os(Linux)
					s = Glibc.sendto(
						self.socketfd, buffer.advanced(by: sent), Int(bufSize - sent), sendFlags, addressPointer,
						addressLength
					)
				#else
					s = Darwin.sendto(
	...
	/// - Returns: Integer representing the number of bytes written.
	///
	@discardableResult public func write(from data: NSData, to _: Address) throws -> Int {
		// Send the bytes...
		try self.write(from: data.bytes.assumingMemoryBound(to: UInt8.self), bufSize: data.length)
	}
	...
	@discardableResult public func write(from data: Data, to address: Address) throws -> Int {
		...
		#if swift(>=5.0)
			return try data.withUnsafeBytes { [unowned self] (buffer: UnsafeRawBufferPointer) throws -> Int in
				try self.write(from: buffer.baseAddress!, bufSize: data.count, to: address)
			}
		#else
			return try data.withUnsafeBytes { [unowned self] (buffer: UnsafePointer<UInt8>) throws -> Int in
				try self.write(from: buffer, bufSize: data.count, to: address)
			}
		#endif
	}
	...
	@discardableResult public func write(from string: String, to address: Address) throws -> Int {
		try string.utf8CString.withUnsafeBufferPointer {
			// The count returned by nullTerminatedUTF8 includes the null terminator...
			try self.write(from: $0.baseAddress!, bufSize: $0.count - 1, to: address)
		}
	}
	...
	///
	public func isReadableOrWritable(waitForever: Bool = false, timeout: UInt = 0) throws -> (
	...
	public func setReadTimeout(value: UInt = 0) throws {
		...
		let result = setsockopt(socketfd, SOL_SOCKET, SO_RCVTIMEO, &timer, socklen_t(MemoryLayout<timeval>.stride))
	...
	public func setWriteTimeout(value: UInt = 0) throws {
		...
		let result = setsockopt(socketfd, SOL_SOCKET, SO_SNDTIMEO, &timer, socklen_t(MemoryLayout<timeval>.stride))
	...
	private func readDataIntoStorage() throws -> Int {
		...
		#else
			self.readBuffer.initialize(to: 0x0, count: self.readBufferSize)
		#endif
	...
	private func readDatagramIntoStorage() throws -> (bytesRead: Int, fromAddress: Address?) {
		...
		do {
			guard
				let address = try Address(addressProvider: { addresssPointer, addressLengthPointer in

					// Read all the available data...
					#if os(Linux)
						let count = Glibc.recvfrom(
							self.socketfd, self.readBuffer, self.readBufferSize, recvFlags, addresssPointer,
							addressLengthPointer
						)
					#else
						let count = Darwin.recvfrom(
							self.socketfd, self.readBuffer, self.readBufferSize, recvFlags, addresssPointer,
							addressLengthPointer
						)
					#endif
				...
				throw Error(
					code: Socket.SOCKET_ERR_WRONG_PROTOCOL,
					reason: "Unable to determine receiving socket protocol family."
				)
			}

			return (self.readStorage.length, address)
		} catch let OperationInterrupted.readDatagram(length) {
			return (length, nil)
		}
	}
	...
	private func wait(forRead: Bool) throws {
		repeat {
			let result = try isReadableOrWritable(waitForever: true)
	...
	private func lastError() -> String {
		String(validatingUTF8: strerror(errno)) ?? "Error: \(errno)"
	}
	...
	///
	private func ignoreSIGPIPE(on _: Int32) throws {
		#if !os(Linux)

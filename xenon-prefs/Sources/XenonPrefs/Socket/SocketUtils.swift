...
extension Socket.Address {
	...
	func withSockAddrPointer<Result>(body: (UnsafePointer<sockaddr>, socklen_t) throws -> Result) rethrows -> Result {
		...
		///
		func castAndCall<T>(_ address: T, _ body: (UnsafePointer<sockaddr>, socklen_t) throws -> Result) rethrows
			-> Result
		{
			var localAddress = address // We need a `var` here for the `&`.
			return try withUnsafePointer(to: &localAddress) {
				return try $0.withMemoryRebound(
					to: sockaddr.self, capacity: 1
				) {
					try body($0, socklen_t(MemoryLayout<T>.size))
				}
			}
		}
...
extension Socket.Address {
		...
		case AF_INET:
			self = withUnsafePointer(to: &addressStorage) {
				$0.withMemoryRebound(to: sockaddr_in.self, capacity: 1) {
					Socket.Address.ipv4($0.pointee)
				}
			}
		case AF_INET6:
			self = withUnsafePointer(to: &addressStorage) {
				$0.withMemoryRebound(to: sockaddr_in6.self, capacity: 1) {
					Socket.Address.ipv6($0.pointee)
				}
			}
		case AF_UNIX:
			self = withUnsafePointer(to: &addressStorage) {
				$0.withMemoryRebound(to: sockaddr_un.self, capacity: 1) {
					Socket.Address.unix($0.pointee)
				}
			}
		default:
			return nil
	...
	extension fd_set {
		...
		mutating func withCArrayAccess<T>(block: (UnsafeMutablePointer<Int32>) throws -> T) rethrows -> T {
			try withUnsafeMutablePointer(to: &__fds_bits) {
	...
	extension fd_set {
		@inline(__always)
		mutating func withCArrayAccess<T>(block: (UnsafeMutablePointer<Int32>) throws -> T) rethrows -> T {
			try withUnsafeMutablePointer(to: &fds_bits) {
				try block(UnsafeMutableRawPointer($0).assumingMemoryBound(to: Int32.self))
			}
		}
	}
...
public extension fd_set {
	...
	mutating func set(_ fd: Int32) {
		...
		self.withCArrayAccess { $0[index] |= mask }
	}
	...
	mutating func isSet(_ fd: Int32) -> Bool {
		let (index, mask) = fd_set.address(for: fd)
		return self.withCArrayAccess { $0[index] & mask != 0 }
	}
}
